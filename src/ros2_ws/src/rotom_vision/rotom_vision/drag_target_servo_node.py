...
class DragTargetServoNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("angular_gain", 0.0)
    ...
    def _joint_state_cb(self, msg: JointState) -> None:
        ...
        if not self._marker_seeded_from_joint_state:
            ...
            if current_pose_world is None:
                return
            self._target_pose_world = current_pose_world.copy()
            self.server.setPose(
                self.marker_name,
                _matrix_to_pose(current_pose_world),
    ...
    def _feedback_cb(self, feedback: InteractiveMarkerFeedback) -> None:
        if feedback.event_type not in (
            ...
            return
        self._target_pose_world = _pose_to_matrix(feedback.pose)
