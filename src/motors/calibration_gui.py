...
# Defaults to allow running without CLI args
DEFAULT_BUS = "feetech"
...
def _build_default_bus() -> MotorsBus:
    ...
    motors = {
        ...
        for name, (mid, model) in DEFAULT_MOTORS.items()
    }
    return BusCls(port=DEFAULT_PORT, motors=motors)


def main_no_cli():
    bus = _build_default_bus()
    groups = DEFAULT_GROUPS if DEFAULT_GROUPS else {"all": list(bus.motors)}
    gui = RangeFinderGUI(bus=bus, groups=groups)
    gui.run()


if __name__ == "__main__":
    # Run with defaults (no CLI) for convenience
    main_no_cli()